...
bool straight, flush, four, three, royal_flush_rank;
int pairs;
...
void analyze_hand(void)
{
    ...
    //checks for a royal flush ranks only, since we have something that checks for flush
...
{
    //adding royal flush
    if(royal_flush_rank && flush) printf("Royal Flush");
